...
import com.example.employeeLeaveApplication.feature.leave.carryforward.component.ApprovalMatrixService;
import com.example.employeeLeaveApplication.feature.leave.carryforward.dto.CarryForwardLeaveApplicationResponse;
import com.example.employeeLeaveApplication.feature.leave.carryforward.dto.CarryForwardLeaveRequest;
...
import com.example.employeeLeaveApplication.feature.leave.carryforward.repository.CarryForwardLeaveApplicationRepository;
...
public class CarryForwardLeaveService {
     ...
     */
    @Transactional
    public CarryForwardLeaveApplicationResponse approveLeave(Long applicationId, Long approverId) {
        ...
        if (approverId.equals(app.getEmployeeId())) {
        ...
        if (isFinalLevel) {
            ...
            CarryForwardBalance balance = cfBalanceRepository
                    .findByEmployeeIdAndYear(app.getEmployeeId(), app.getYear())
                    .orElseThrow(() -> new RuntimeException(
            ...
            CarryForwardLeaveApplication saved = cfLeaveAppRepository.save(app);
            Employee applicant = employeeRepository.findById(app.getEmployeeId()).orElse(null);
            ...
            CarryForwardBalance balance = cfBalanceRepository
                    .findByEmployeeIdAndYear(app.getEmployeeId(), app.getYear()).orElse(null);
    ...
    @Transactional
    public CarryForwardLeaveApplicationResponse rejectLeave(Long applicationId, Long rejectorId, String reason) {
    ...
    @Transactional
    public CarryForwardLeaveApplicationResponse cancelLeave(Long applicationId, Long employeeId) {
        ...
        if (app.getStatus() == LeaveStatus.REJECTED || app.getStatus() == LeaveStatus.CANCELLED) {
            throw new RuntimeException("Application is already " + app.getStatus());
        }

        boolean wasApproved = (app.getStatus() == LeaveStatus.APPROVED);
    ...
    public CarryForwardLeaveApplicationResponse getApplication(Long applicationId) {
        ...
        Employee approver  = app.getApprovedBy() != null
                ? employeeRepository.findById(app.getApprovedBy()).orElse(null) : null;
        Employee rejector  = app.getRejectedBy() != null
                ? employeeRepository.findById(app.getRejectedBy()).orElse(null) : null;
        CarryForwardBalance balance = cfBalanceRepository
                .findByEmployeeIdAndYear(app.getEmployeeId(), app.getYear()).orElse(null);
        double remaining = balance != null && balance.getRemaining() != null
                ? balance.getRemaining() : 0.0;
    ...
    @Transactional(readOnly = true)
    public List<CarryForwardLeaveApplicationResponse> getMyApplications(Long employeeId) {
        Employee employee = employeeRepository.findById(employeeId).orElse(null);
        return cfLeaveAppRepository.findByEmployeeId(employeeId).stream()
                .map(app -> {
                    CarryForwardBalance bal = cfBalanceRepository
                            .findByEmployeeIdAndYear(employeeId, app.getYear()).orElse(null);
     ...
     */
    @Transactional(readOnly = true)
    public List<CarryForwardLeaveApplicationResponse> getPendingForApprover(Long approverId) {
        Employee approver = employeeRepository.findById(approverId)
                .orElseThrow(() -> new RuntimeException("Approver not found: " + approverId));
    ...
    public List<CarryForwardLeaveApplicationResponse> getAllPending() {
        return cfLeaveAppRepository.findByStatus(LeaveStatus.PENDING).stream()
                .map(app -> {
                    Employee applicant = employeeRepository.findById(app.getEmployeeId()).orElse(null);
                    CarryForwardBalance bal = cfBalanceRepository
                            .findByEmployeeIdAndYear(app.getEmployeeId(), app.getYear()).orElse(null);
    ...
    @Transactional
    public void processYearEndCarryForward(Long employeeId, Integer fromYear, double annualLeaveRemaining) {

        int toYear = fromYear + 1;
        log.info("[CF-YEAR-END] employee={}, fromYear={}, annualLeaveRemaining={}",
                employeeId, fromYear, annualLeaveRemaining);

        double carryAmount = Math.min(annualLeaveRemaining, PolicyConstants.MAX_CARRY_FORWARD);
        ...
        CarryForwardBalance existing = cfBalanceRepository
                .findByEmployeeIdAndYear(employeeId, toYear).orElse(null);
    ...
    private String resolveRole(Employee employee) {
        if (employee.getRole() == null) {
            throw new RuntimeException("Employee " + employee.getId() + " has no role assigned");
        }

        return employee.getRole().name(); // enum → String
    }
    ...
    private CarryForwardLeaveApplicationResponse toResponse(
        ...
        Employee level1Approver = app.getLevel1ApprovedBy() != null
                ? employeeRepository.findById(app.getLevel1ApprovedBy()).orElse(null)
                : null;

        CarryForwardLeaveApplicationResponse r = new CarryForwardLeaveApplicationResponse();
        r.setId(app.getId());
        r.setEmployeeId(app.getEmployeeId());
